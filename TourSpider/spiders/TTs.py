...
class TourspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        print("国家抓取完成")
        curl='http://www.mafengwo.cn/jd/21536/gonglve.html'
        cou='中国'
        url='21536'
        current_count = 1
        yield response.follow(
    ...
    def after_post_getjd(self, response):
        ...
        for i in range(len(jd_url)):
            ...
            yield FormRequest(url=sjd_url, headers=headers, callback=self.get_comment_count,
                              ...
                              dont_filter=True)
            '''
            sjd_url = 'http://pagelet.mafengwo.cn/poi/pagelet/poiCommentListApi?callback=jQuery181011702454390918593_1525247201501&params={"poi_id":' + \
                      jd_item['url'] + '}&_=1525247201612'
            headers = {
                'Accept - Encoding': 'gzip, deflate',
                'Accept - Language': 'en - US, en;q=0.5',
                'Connection': 'keep - alive',
    ...
    def get_comment_count(self, response):
        ...
        for comm_countc in range(comm_all_count):
            ...
            if comm_count == 1:
                ...
                sjd_url = 'http://pagelet.mafengwo.cn/poi/pagelet/poiCommentListApi?callback=jQuery18103080186012365017_1524305195722&params={"poi_id":' + jd_url + ',"page":' + str(
                    comm_count) + ',"just_comment":1}&=1524305229292'
